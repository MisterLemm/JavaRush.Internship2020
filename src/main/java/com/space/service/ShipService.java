...
import com.space.exceptionhandler.BadRequestException;
import com.space.exceptionhandler.NotFoundException;
...
public class ShipService {
    ...
    public Ship update(Ship ship, Long id) {
        Ship update = getShipById(id);

        if (ship == null || update == null) {
            throw new BadRequestException();
        }
        ...
        update.setRating(calculateRating(update));
        return shipRepository.save(update);
    }

    public List<Ship> getListOfShips(String name, String planet, ShipType shipType, Long after, Long before,
        ...
        if (name != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getName().contains(name)).collect(Collectors.toList());
        }
        if (planet != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getPlanet().contains(planet)).collect(Collectors.toList());
        }
        if (shipType != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getShipType().equals(shipType)).collect(Collectors.toList());
        }
        if (after != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getProdDate().after(new Date(after))).collect(Collectors.toList());
        }
        if (before != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getProdDate().before(new Date(before))).collect(Collectors.toList());
        }
        if (getUsed != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getUsed().equals(getUsed)).collect(Collectors.toList());
        }
        if (minSpeed != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getSpeed() >= minSpeed).collect(Collectors.toList());
        }
        if (maxSpeed != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getSpeed() <= maxSpeed).collect(Collectors.toList());
        }
        if (minCrewSize != null) {
            filteredShips = filteredShips.stream()
        ...
        if (maxCrewSize != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getCrewSize() <= maxCrewSize).collect(Collectors.toList());
        }
        if (minRating != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getRating() >= minRating).collect(Collectors.toList());
        }
        if (maxRating != null) {
            filteredShips = filteredShips.stream()
                    .filter(ship -> ship.getRating() <= maxRating).collect(Collectors.toList());
        }
    ...
    public List<Ship> filteredShips(final List<Ship> shipList, ShipOrder order, Integer pageNumber, Integer pageSize) {
        ...
        return shipList.stream().sorted(getComparator(order)).skip(pageNumber * pageSize).limit(pageSize)
    ...
    private Double calculateRating(Ship ship) {
        ...
        double condition = ship.getUsed() ? 0.5d : 1.0d;
        int date = 3019;
        int productionDate = ship.getProdDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear();
        double rating = (80 * speed * condition) / (double) (date - productionDate + 1);
